        ...
        @if (@$attachment->type == 'image')
            <div class="image-wrapper" style="text-align: {{ $isSender ? 'end' : 'start' }}">
                <div class="image-file chat-image">
                    <img id="image" src="" alt="{{ $attachment->title }}" class="chat-image-preview">

                    <div class="image-info">{{ $attachment->title }}</div>
                </div>
...
<script>
    var jsonData = {!! json_encode(['attachmentUrl' => Chatify::getAttachmentUrl($attachment->file)]) !!};
